...
(function () {
  ...
  function decorateUpdates(root) {
    ...
    for (var i = 0; i < names.length; i++) {
      ...
      badge.textContent = "?";
      badge.title = "Update available";                 // native fallback
      badge.setAttribute("data-tip", "Update available"); // our CSS tooltip
  ...
  function injectBadgeStyle() {
    ...
    s.id = "ut-badge-style";
    s.textContent =
      ".ut-update-badge{position:relative;display:inline-flex;align-items:center;justify-content:center;" +
      "width:14px;height:14px;margin-left:3px;border-radius:50%;font-size:10px;" +
      ...
      // custom tooltip (native title is unreliable in the Docker view)
      ".ut-update-badge:hover::after{content:attr(data-tip);position:absolute;left:50%;" +
      "bottom:calc(100% + 7px);transform:translateX(-50%);background:#1c1c1c;color:#fff;" +
      "border:1px solid rgba(255,255,255,0.18);padding:4px 8px;border-radius:4px;font-size:11px;" +
      "font-weight:400;white-space:nowrap;z-index:99999;pointer-events:none;" +
      "box-shadow:0 3px 10px rgba(0,0,0,0.5);}" +
      ".ut-update-badge:hover::before{content:'';position:absolute;left:50%;bottom:calc(100% + 2px);" +
      "transform:translateX(-50%);border:5px solid transparent;border-top-color:#1c1c1c;" +
