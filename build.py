...
- Themes now show their palette. Every entry in the theme store and the preset
  ...
  accent 2), so you can see what a theme looks like before downloading it.
- Fixed unreadable button labels in eleven bundled themes. Filled buttons paint
  the accent as their background while the label was hard-coded white, which on
  Nord, Dracula, Catppuccin Mocha, Tokyo Night, One Dark and Gruvbox meant a
  contrast ratio around 2:1 against the WCAG AA minimum of 4.5. Labels now take
  the theme's own background while that clears AA, otherwise black or white.
- The theme builder shows live WCAG contrast for the five pairs that carry text
  and picks a readable button label for you instead of always writing white.
- Themer Icons is navigable: a filter bar with search (label, slot key or the
  set's default icon name), a category filter, an "Only customised" toggle and a
  slot counter. The table scrolls in its own box so the column headers stay put.
- Reset a SINGLE icon back to the set default with the eraser button on its row.
  Previously the only way back was "Reset all", which wiped every override.
- Store rows line up: the three button states have different label widths, which
  used to push every row's swatch and name to a different position.
- A beta channel now exists for testing new features ahead of a release, see the
  README. Installing it is a normal plugin install and your settings carry over.
